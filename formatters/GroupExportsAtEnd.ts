...
class Formatter implements f.Formatter
{
	...
	private _collectAllExportStatements(): void
	{
		...
		for (let i = this.sourceFile.statements.length - 1; i >= 0; i--)
		{
			...
			this._isExportAssignment(statement) && counter.assignments++;
			this._isExportDeclaration(statement) && counter.declarations++;
		}
		...
		if(counter.assignments !== 0 || counter.declarations > 1)
		{
			...
			{
				const statement = this.sourceFile.statements[i];
				this._isExportAssignment(statement) && this._cutExportAssignment(statement, i);
				this._isExportDeclaration(statement) && this._cutExportDeclaration(statement, i);
			}
		}
	}
	...
	private _insertUnifiedExportDeclaration(): void
	{
		...
		if(this.exportDeclarations.length)
		{
			...
			exportedEntities.forEach((item: any, index: any) =>
			{
				...
				const lineEnding = (lastItem ? this.formatOptions.NewLineCharacter
											 : "," + this.formatOptions.NewLineCharacter);

				const tsLintIndentation = this.formatOptions.TsLintRules["indent"] ? this.formatOptions.TsLintRules["indent"][1] : null;
				const defaultIndentation: string = tsLintIndentation === "tabs" ? this.formatOptions.TabCharacter
																				: this.formatOptions.FourSpacesIndent;

				unifiedExport += defaultIndentation + item + lineEnding;
			});

			unifiedExport += '};';

			this.edits.push({
				span: { start: eof, length: 0 },
				newText: this.formatOptions.NewLineCharacter + this.formatOptions.NewLineCharacter + unifiedExport, priority: 0
			});
		}
	}

	private _isExportDeclaration(node: ts.Node): boolean
	{
		return node.kind === ts.SyntaxKind.ExportDeclaration;
	}

	private _isExportAssignment(node: ts.Node): boolean
	{
		const isExportAssignment = (node.kind === ts.SyntaxKind.ExportAssignment
		|| Lint.hasModifier(node.modifiers, ts.SyntaxKind.ExportKeyword))
		&& !Lint.hasModifier(node.modifiers, ts.SyntaxKind.DefaultKeyword);

		return isExportAssignment;
	}
}
